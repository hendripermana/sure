...
export default class extends Controller {
  ...
  close() {
    ...
    setTimeout(() => {
      if (!this.isOpen && this.hasMenuTarget) this.menuTarget.classList.add("hidden");
    }, 150);
  }
  ...
  scrollToSelected() {
    ...
    const delta =
      selectedRect.top - containerRect.top - (container.clientHeight - selectedRect.height) / 2;
  ...
  handleKeydown(event) {
    ...
    switch (event.key) {
      case "ArrowDown":
        nextIndex = currentIndex < 0 ? 0 : (currentIndex + 1) % visibleOptions.length;
        break;
      case "ArrowUp":
        nextIndex =
          currentIndex < 0
  ...
  rovingFocus(visibleOptions, index) {
    ...
    for (const opt of all) opt.setAttribute("tabindex", opt === target ? "0" : "-1");
    target.focus();
  }
  ...
  syncTabindex() {
    ...
    if (tabbable) return;
    const all = this.hasOptionTarget ? this.optionTargets : [];
    for (const opt of all) opt.setAttribute("tabindex", "-1");
